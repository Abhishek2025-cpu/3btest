...
exports.createItemWithBoxes = async (req, res) => {
  try {
    const { 
      ...
      length, 
      noOfSticks, 
      helperEids = [], // ✅ now array
      operatorEids = [], // ✅ now array
      shift, 
      company,
      noOfBoxes
    } = req.body;
    ...
    const existingItem = await MainItem.findOne({ itemNo });
    if (existingItem) {
      return res.status(409).json({ error: `Item with itemNo '${itemNo}' already exists.` });
    }

    // --- 2️⃣ Fetch multiple employees ---
    const [helpers, operators, productImageUpload] = await Promise.all([
      Employee.find({ eid: { $in: helperEids } }),
      Employee.find({ eid: { $in: operatorEids } }),
      uploadBufferToGCS(req.file.buffer, req.file.originalname, 'product-images', req.file.mimetype)
    ]);

    if (helpers.length === 0 || operators.length === 0) {
      return res.status(400).json({ error: 'Invalid helper(s) or operator(s) EID(s).' });
    }

    // --- 3️⃣ Generate boxes & QR codes ---
    const boxIndexes = Array.from({ length: numBoxes }, (_, i) => i + 1);
    const generatedBoxes = await Promise.all(
      boxIndexes.map(async (index) => {
    ...
    const newMainItem = await MainItem.create({
      ...
      length,
      noOfSticks,
      helpers: helpers.map(h => ({ _id: h._id, name: h.name, eid: h.eid })),
      operators: operators.map(o => ({ _id: o._id, name: o.name, eid: o.eid })),
      shift,
      company,
      productImageUrl: productImageUpload.url,
      boxes: generatedBoxes,
      pendingBoxes: numBoxes, // ✅ default equal to noOfBoxes
      completedBoxes: 0        // ✅ start with 0
    });
